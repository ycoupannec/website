	...
	switch($mode) {
		...
		case 'collections': //does both collections and series from drupal
			...
			foreach($terms as $t) {
				...
				$wpdb->query('INSERT INTO `wp_term_taxonomy` (term_id, taxonomy, parent) VALUES ("' . $t["tid"] . '", "collections", "' . $parent . '")');
...
function kb_add_term($cid,$taxonomy) {
	...
	if(!empty($t)) {
		...
		$wpdb->query('INSERT INTO `wp_term_taxonomy` (term_id, taxonomy, parent) VALUES ("' . $t["tid"] . '", $taxonomy, "' . $parent . '")');
